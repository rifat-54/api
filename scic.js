const students = [
  ...
  const scic=(person)=>person.filter(a=>a.avg>48 && a.fiftyPercent).map(a=>a.name);
        // return result.map(a=>a.name)


  const ans=scic(students);
  console.log(ans);

//   const result=students.filter(a=>a.avg>48 && a.fiftyPercent)
//   const ans=result.map(a=>a.name)
//   console.log(ans)
